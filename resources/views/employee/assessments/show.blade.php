        ...
        <div class="bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-lg rounded-xl shadow-xl overflow-hidden border border-gray-700/30">
            <div class="p-8">
                ...
                <!-- {{ $assessment->getIsExpiredAttribute() ? 'Expired' : 'Active' }} -->
                    ...
                    <!-- check due date is past -->
                     @if($assessment->due_date && $assessment->due_date < now())
                        <span class="px-2.5 py-0.5 inline-flex text-md leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                            Expired
                        </span>
                    @endif

                    <!-- check if assessment is published -->
                    @if($assessment->due_date && $assessment->due_date > now() && $assessment->is_published)
