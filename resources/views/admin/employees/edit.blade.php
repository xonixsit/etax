...
<main class="flex-1 max-w-4xl px-4 py-6 mx-auto w-full">
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-semibold text-gray-100">Edit Employee</h1>
                    <p class="text-sm text-gray-400">Update employee information</p>
                </div>
            </div>
    ...
    <div class="w-full mx-auto">
        <div class="bg-slate-800/60 backdrop-blur-md rounded-lg shadow-lg border border-slate-700/50 overflow-hidden">
            <form method="POST" action="{{ route('admin.employees.update', $employee) }}" novalidate class="space-y-6" aria-labelledby="form-title">
                @csrf
                @method('PUT')
                @if ($errors->any())
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="form-group">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="department" id="department" class="block w-full pl-9 pr-3 py-2 text-sm rounded-lg bg-slate-900/50 border border-slate-700/50 text-gray-300 placeholder-gray-500 focus:border-indigo-500 focus:ring focus:ring-indigo-500/20 transition-all duration-150 @error('department') border-red-500/50 focus:border-red-500 focus:ring-red-500/20 @enderror" placeholder="e.g., Sales" value="{{ old('department', $employee->department) }}" required>
                                @error('department')
                                <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
